...
export async function createPayment({ amount, description, orderId, customerName, customerPhone, paymentMethod, appUrl }) {
  try {
    ...
    const payload = {
      ...
      webhook_url: `${runtimeAppUrl}/api/payments/webhook`, // Compatibilité
      success_url: `${runtimeAppUrl}/api/payments/success?order=${orderId}`,
      callback_success: `${runtimeAppUrl}/api/payments/success?order=${orderId}`, // Compatibilité
      cancel_url: `${runtimeAppUrl}/api/payments/cancel?order=${orderId}`,
      callback_cancel: `${runtimeAppUrl}/api/payments/cancel?order=${orderId}` // Compatibilité
    };
